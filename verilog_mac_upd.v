...
//=========================================================
// 3x3 PARALLEL MAC ACCELERATOR - BATCHED BURST MODE
// 64-bit AXI-Stream in, 32-bit AXI-Stream out
// Processes 9 x/weights per MAC, streams results back
//=========================================================
...
module mac_3x3_accelerator(
    ...
    reg [1:0]  beat_count;
    reg        last_packet_reg;          // Propagates DMA tlast to output
            ...
            case (state)
                ...
                S_RECEIVE: begin
                    if (s_axis_tvalid && s_axis_tready) begin
                        ...
                        // Auto-detect 3rd beat for continuous streaming
                        if (beat_count == 2'b10) begin
                            state           <= S_OUTPUT;
                            beat_count      <= 2'b00;
                            last_packet_reg <= s_axis_tlast;  // Capture end-of-burst
                        end else begin
    ...
    assign m_axis_tlast  = m_axis_tvalid && last_packet_reg;  // ONLY on final result
